        ...
        <div className="container mx-auto max-w-4xl py-8 px-4">
          ...
          <Card className="border-2">
            <CardHeader>
              ...
              <CardDescription>
                {currentStep === 1 && "Pick your adventure. We're the sherpa, you're the climber. Choose how much heavy lifting you want us to do."}
                {currentStep === 2 && "Tell us about your LLC. We'll write it down on the official forms and walk them to the right desk."}
                {currentStep === 3 && "Who's running this show? We need contact info so we can let you know when your paperwork is ready."}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form}>
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <div className="grid gap-4">
                              {packages.map((pkg) => {
                                const isSelected = field.value === pkg.name;
                                return (
                                  <div
                                    key={pkg.name}
                                  ...
                                  >
                                    <div className="flex justify-between items-start mb-4">
                                      <div className="flex-1">
                                        <h3 className={`font-display text-xl font-bold mb-2 ${isSelected ? 'text-white' : 'text-primary'}`}>
                                          {pkg.title}
                                        </h3>
                                        <p className={`text-sm leading-relaxed ${isSelected ? 'text-white/95' : 'text-gray-600'}`}>
                                          {pkg.description}
                                        </p>
                                      </div>
                                      <div className="ml-4 text-right">
                                        <span className={`text-4xl font-bold ${isSelected ? 'text-white' : 'text-accent'}`}>
                                          ${pkg.price}
                                        </span>
                                      </div>
                                    </div>
                                    <ul className="space-y-2 mt-4">
                                      {pkg.features.map((feature, idx) => (
                                        <li key={idx} className="flex items-start gap-2">
                                          <svg className={`w-5 h-5 flex-shrink-0 mt-0.5 ${isSelected ? 'text-white/80' : 'text-accent'}`} fill="currentColor" viewBox="0 0 20 20">
